...
IG_COLUMN_ORDER = [
    "Thời gian đăng", "Nội dung hiển thị", "Lượt thích", "Lượt xem", "Số người tiếp cận",
    "Lượt chia sẻ", "Lượt theo dõi", "Bình luận", "Lượt lưu", "Thời lượng (giây)", "Loại bài viết"
]
# ==========================================
...
with tab1:
    ...
    if valid_dfs:
        merged = valid_dfs[0]
        for next_df in valid_dfs[1:]: merged = pd.merge(merged, next_df, on="Ngày", how="outer")
...
with tab2:
    if fb_df is not None:
        ...
        remaining = [c for c in display_fb_df.columns if c not in existing_ordered and c not in ['ID', 'Ngày', 'Liên kết vĩnh viễn', 'Tiêu đề', 'Mô tả']]
        display_fb_df = display_fb_df[existing_ordered + remaining]
...
with tab3:
    if ig_df is not None:
        ...
        remaining = [c for c in display_ig_df.columns if c not in existing_ordered and c not in ['ID', 'Ngày', 'Liên kết vĩnh viễn', 'Tiêu đề', 'Mô tả']]
        display_ig_df = display_ig_df[existing_ordered + remaining]
